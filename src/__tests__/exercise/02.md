...
When we get a warning about act() in our test, we have to consider what lines are updating a state in our test and those lines need to be wrapped with act() https://legacy.reactjs.org/docs/testing-recipes.html#act 
Article on how to fix act() warning (warnings) https://kentcdodds.com/blog/fix-the-not-wrapped-in-act-warning 
The Testing Library can extend Jest as @testing-library/jest-dom http://testing-library.com/jest-dom  -> the new library is here https://github.com/testing-library/jest-dom library provides a set of custom jest matchers that you can use to extend jest.
