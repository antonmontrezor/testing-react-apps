...
When writing tests we shouldn’t care about implementation details of our abstractions, we shouldn’t test that. We need to make sure that if the implementation changes our test pass and are in tact. The user does not observe or care about differences in implementations, so we should write our tests in a way that passes in either case.

https://testing-playground.com/ a tool used to find the best query for your piece of code. Docs about how to make queries of DOM elements https://testing-library.com/docs/queries/about/ screen.debug(container) -> is a very nice feature

This block 03js is about: exercise is intended to help you learn to avoid implementation details by querying for and interacting with the elements in a way that is implementation detail free and refactor friendly.

To free our test from implementation details, we need to query elements accordingly using https://testing-library.com/docs/queries/about/ , we mimic the way a user chooses and interacts with elements in our tests that’s why we sticky to the query priorities where the top one is the most accessible

user-event as a more powerful alternative to fireEvent https://testing-library.com/docs/user-event/intro  we should prefer using userEvent over fireEvent as if the event in implementattion changes from onClick to mouseUp, it won't break our tests
