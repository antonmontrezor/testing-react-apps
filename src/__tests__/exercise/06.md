...
When mocking an API, check its implementation details in the docs. Also, if you use a hook or another third-party package that includes the API you want to mock, like
getCurrentPosition, which is a part of react-use-geolocation, then go through each file that is imported or installed in your project related to that third-party package to see how the 
API you want to mock is called/used.

Additional info on act()
act() helps to flush a side effect before moving on with the test. This ensures that we are testing the behaviour the user would see in the browser, having no discrepancies between how our tests perceive the changes in the DOM (side effects) (my understanding)
