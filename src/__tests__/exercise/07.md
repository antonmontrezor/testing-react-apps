...
It’s recommended to not import anything from the testing library directly into our tests, we should create a module that would export everything from the testing library and with our custom render function

Here’s the way to not use ‘../../‘ in imports but to configure Jest the way we can specify modules as node modules -> import {render, screen} from 'test/test-utils'. we need to specify moduleDirectories property

It's great that we can leverage render and rerender functions with custom wrappers to pass the necessary context values to tested components
